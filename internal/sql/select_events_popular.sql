WITH teams_with_participants_count AS (
  SELECT fsp.users_x_teams.team AS team, count(*) AS participants 
  FROM fsp.users_x_teams 
  GROUP BY fsp.users_x_teams.team
), events_with_participants_count AS (
  SELECT fsp.events_x_teams.event AS event, SUM(teams_with_participants_count.participants) AS participants
...
SELECT fsp.events.id, fsp.events.name, fsp.events.owner, fsp.events.description, fsp.events.start_date, fsp.events.end_date, fsp.events.min_participants, fsp.events.max_participants
FROM fsp.events
